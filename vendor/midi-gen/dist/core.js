...
var CHAR_VALUE = new Map(
  [...ALPHABET].map((ch, i) => [ch, i])
);
CHAR_VALUE.set("O", 0);
CHAR_VALUE.set("I", 1);
CHAR_VALUE.set("L", 1);
var SEED_MASK = (1n << 60n) - 1n;
var CODE_CHARS = 16;
...
function crc8(bytes) {
  ...
  for (const byte of bytes) {
    crc ^= byte;
    for (let i = 0; i < 8; i++) {
      crc = crc & 128 ? (crc << 1 ^ 7) & 255 : crc << 1 & 255;
    }
  }
  return crc;
}
function payloadBytes(payload72) {
...
function encodeCode(genre, seed, opts = {}) {
  ...
  const payload72 = BigInt(version) << 68n | BigInt(genreIndex) << 63n | BigInt(flags) << 60n | seed & SEED_MASK;
...
function decodeCode(input) {
  ...
  for (const ch of raw) {
    ...
    if (v === void 0) throw new CodeError(`invalid character: ${ch}`, "charset");
...
// src/core/gen/form.ts
function buildForm(cfg, rng, targetBars) {
  let template = rng.weighted(cfg.structures);
  if (targetBars) template = stretchTemplate(template, targetBars);
  const sections = [];
  let bar = 0;
  for (const spec of template) {
    const section = { name: spec.name, startBar: bar, bars: spec.bars };
...
function stretchTemplate(template, targetBars) {
  const isEdge = (name) => name === "intro" || name === "outro";
...
// src/core/gen/title.ts
function buildTitle(spec, rng) {
  const pattern = rng.weighted(spec.patterns);
  const used = /* @__PURE__ */ new Map();
  return pattern.replace(/\{([a-z]+)(\d*)\}/g, (_m, slot, _idx) => {
...
// src/core/gen/harmony.ts
function buildHarmony(sections, cfg, key, barTicks, beatTicks, rng) {
  const beatsPerBar = barTicks / beatTicks;
  const templateByName = /* @__PURE__ */ new Map();
  const spans = [];
...
function makeChordLookup(spans) {
  return (tick) => {
...
// src/core/gen/bass.ts
function placeInRegister(pc, lo, hi) {
  for (let p = lo; p <= hi; p++) {
    if (mod12(p) === pc) return p;
  }
  return lo;
}
function nearestInRegister(pc, prev, lo, hi) {
  ...
  for (let si = 0; si < ctx.chords.length; si++) {
    ...
    for (let bar = 0; bar < bars; bar++) {
      ...
      for (let h = 0; h < hits.length; h++) {
        ...
        const nextStart = h + 1 < hits.length ? barStart + Math.round(hits[h + 1] * ctx.beatTicks) : barStart + ctx.barTicks;
        let pitch = root;
        if (h > 0 && rng.chance(0.18)) pitch = placeInRegister(fifthPc, lo, hi);
        else if (h > 0 && rng.chance(0.12)) pitch = root + 12 <= hi ? root + 12 : root;
        else if (h > 0 && root - 12 >= lo && rng.chance(0.1)) pitch = root - 12;
        ...
        notes.push({
          pitch,
          start,
          dur: nextStart - start - 10,
          // near-legato → portamento glide
...
var genBass = (ctx) => {
  ...
  if (!inst) return null;
  const style = ctx.cfg.bass.styles ? ctx.rng("bass").weighted(ctx.cfg.bass.styles) : ctx.cfg.bass.style;
  let notes;
  switch (style) {
    case "walking":
      notes = walking(ctx);
...
// src/core/gen/comping.ts
function arpCycle(pattern, voicing, hi) {
...
// src/core/gen/arrange.ts
function sectionAt(sections, barTicks, tick) {
...
function arrange(tracks, ctx) {
  const { sections, barTicks, cfg } = ctx;
  const total = ctx.totalBars * barTicks;
  const layersOf = (name) => cfg.arrange.layers[name] ?? "all";
  const isActive = (role, layers) => layers === "all" || layers.includes(role);
  return tracks.map((track) => {
    const kept = track.notes.filter((n) => {
...
// src/core/gen/humanize.ts
function humanize(tracks, ctx) {
...
function generate(opts = {}) {
  ...
  const swing = cfg.swing[0] === cfg.swing[1] ? cfg.swing[0] : cfg.swing[0] + global.next() * (cfg.swing[1] - cfg.swing[0]);
  const title = buildTitle(cfg.naming, rng("title"));
  const timeSig = cfg.timeSig;
  const beatTicks = PPQ * 4 / timeSig[1];
  const barTicks = beatTicks * timeSig[0];
  const targetBars = minutes ? Math.max(1, Math.round(minutes * 60 * bpm * PPQ / 60 / barTicks)) : void 0;
...
function songToMidi(song) {
  ...
  midi.header.name = `${song.title} \xB7 ${song.code}`;
